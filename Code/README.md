...
> **Instructions for screenshots:** Run the commands listed under each screenshot number below. Take a screenshot of your terminal after each one. The screenshots you take on your VM replace this placeholder section.

### Screenshot 1 — Multi-Container Supervision
**What to show:** Two or more containers running under one supervisor.

```bash
# Terminal 1 (supervisor already running)
...
**Caption:** *Two containers (alpha, beta) launched under a single supervisor process. Supervisor shows host PIDs for both.*

---

### Screenshot 2 — Metadata Tracking
**What to show:** `engine ps` output with tracked container metadata.
...
**Caption:** *`engine ps` showing both containers with state, memory limits, nice value, and start time.*

---

### Screenshot 3 — Bounded-Buffer Logging
**What to show:** Log file contents captured through the pipeline; supervisor shows producer/consumer activity.

```bash
sudo ./engine logs alpha
# Also show supervisor terminal where producer/consumer messages appear
cat logs/alpha.log
```

**Caption:** *Log file for container `alpha` captured via pipe → bounded buffer → consumer thread → log file. Supervisor shows "[producer] Started" and "[consumer] Logging thread started" messages.*

---

### Screenshot 4 — CLI and IPC
**What to show:** A CLI command reaching the supervisor and getting a response (demonstrating UNIX socket IPC).
...
**What to show:** `dmesg` showing soft-limit warning from the kernel module.
...
**Caption:** *Kernel module emits SOFT LIMIT warning when container `mem` exceeds 20 MiB RSS. Warning appears exactly once.*

---

### Screenshot 6 — Hard-Limit Enforcement
**What to show:** `dmesg` showing hard-limit kill; `engine ps` showing `hard_limit_killed` state.
...
**Caption:** *Kernel module sends SIGKILL when container exceeds 40 MiB hard limit. Supervisor classifies exit as `hard_limit_killed` (SIGKILL with no `stop_requested`).*

---

### Screenshot 7 — Scheduling Experiment
**What to show:** Two CPU-bound containers with different nice values; observable difference in completion speed.
...
**Caption:** *Container `hi` (nice=-5, higher priority) completes more CPU iterations per second than `lo` (nice=15). CFS weight difference of ~40:1 causes clearly unequal CPU share.*
...
**Caption:** *Supervisor cleanly shuts down — producer threads join after pipe EOF, consumer thread drains buffer, no zombie processes, kernel module unloads successfully.*
