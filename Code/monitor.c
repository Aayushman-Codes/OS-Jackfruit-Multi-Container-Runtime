 ...
 *
 * Design choices:
 *   - mutex (not spinlock): timer callback can sleep between list ops,
 *     and ioctl runs in a sleepable context (process context). A spinlock
 *     would prevent any sleeping and is unnecessary here.
 *   - list_for_each_entry_safe used in timer callback to allow safe
 *     deletion of entries during iteration (avoids use-after-free).
 */
...
static void timer_callback(struct timer_list *t)
{
    ...
    list_for_each_entry_safe(entry, tmp, &monitored_list, list) {
        ...
        /* Hard limit: terminate and remove */
